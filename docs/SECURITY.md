...
- Галерея автомобиля использует отдельные permissions `vehicles.photos.view/upload/manage`, повторно проверяет tenant и branch на сервере и ограничена 100 файлами на автомобиль. Original и три производных объекта создаются с уникальным attempt ID; EXIF orientation применяется до удаления всей metadata, включая геолокацию. HEIC/HEIF отклоняется понятной ошибкой до появления проверенного server-side decoder.
- Импорт фото завершённого осмотра не копирует original: `VehicleMedia.SourceInspectionPhotoId` и tenant-aware FK ссылаются на доказательство, а gallery metadata владеет только thumbnail/medium/large. Очередь удаления получает только объекты, которыми владеет удаляемая запись; недоступность MinIO не откатывает уже подтверждённое удаление metadata.
