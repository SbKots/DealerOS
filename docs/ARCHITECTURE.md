...
`VehicleMedia` — единая модель приватной галереи автомобиля и медиа объявления. Сервер декодирует реальное содержимое JPEG/PNG/WebP, применяет EXIF orientation и перекодирует его без EXIF/GPS; лимиты — 8 МиБ и 25 MP. Для каждой upload-attempt создаются уникальные ключи original/thumbnail/medium/large, поэтому проигравший конкурентный запрос удаляет только собственные объекты. Производные варианты ограничены длинной стороной 480/1280/2560 px и никогда не увеличиваются.

Категории: `MainView`, `Front`, `Rear`, `LeftSide`, `RightSide`, `Interior`, `Dashboard`, `Trunk`, `Engine`, `Documents`, `Defect`, `Other`. Подпись, focal point, порядок, обложка и выбор для объявления защищены optimistic concurrency и аудитом; `Documents` нельзя сделать обложкой или включить в объявление. Фото завершённого осмотра импортируется ссылкой на неизменяемый original object: галерея владеет только своими производными вариантами, а tenant-aware FK сохраняет источник. Поэтому удаление gallery metadata не может удалить доказательство осмотра.

`ListingContent` хранит snapshot характеристик, контента, цены, template/version и выбранных упорядоченных media. `Ready` неизменяем, требует `MainView`/`Interior`/`Defect` и ровно одну cover; после фиксации snapshot отбор фотографий не меняется. Manual export создаёт только `Exported`; `Published` появляется исключительно отдельной командой после фактического подтверждения сотрудником. Command ID, optimistic concurrency, history и audit защищают историю цены, контента и публикаций.
