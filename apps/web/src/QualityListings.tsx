...
export function QualityListingsWorkspace() {
  ...
  return <>
    <div className="page-heading"><div><p className="eyebrow">Итерация 0.5</p><h1>Качество и публикационная готовность</h1><p className="muted">ReadyForSale появляется только после независимого QC; export не выдаётся за публикацию.</p></div></div>
...
function QualityWorkspace() {
  ...
  return <section className="panel"><div className="panel-title"><span className="step">05</span><div><h2>Независимый контроль качества</h2><p>Каждая повторная проверка сохраняется отдельной ревизией.</p></div></div>
    ...
    {!quality ? queue.isLoading ? <p>Загрузка…</p> : queue.data?.length ? <div className="queue-grid">{queue.data.map((item) => <article className="queue-card" key={item.executionId}><span className="status draft"><i />{item.latestQualityStatus ?? 'Ожидает QC'}</span><h3>{item.make} {item.model}</h3><p className="mono">{item.vin}</p><dl><div><dt>Факт подготовки</dt><dd>{money(item.actualTotalAmount, item.currency)}</dd></div><div><dt>Работ</dt><dd>{item.workOrders.length}</dd></div></dl><button className="primary" onClick={() => open(item)}>Открыть QC</button></article>)}</div> : <div className="empty-state"><strong>Очередь QC пуста</strong><p>Нет завершённых execution, ожидающих решения.</p></div> : <div>
      <button className="link-button" onClick={() => { setQuality(null); setSelected(null) }}>← К очереди</button><div className="work-card-head"><div><span className="status draft"><i />QC rev. {quality.revision} · {quality.status}</span><h3>Checklist snapshot и замечания</h3></div></div>
      <pre className="snapshot-preview">{JSON.stringify(JSON.parse(quality.checklistSnapshotJson), null, 2)}</pre>
      {quality.observations.map((x) => <div className="critical-banner" key={x.id}><strong>{x.severity}</strong><span>{x.comment}</span></div>)}
      {quality.status === 'Draft' && selected && <><div className="work-form"><label>Работа<select aria-label="Работа для возврата" value={workOrderId} onChange={(e) => setWorkOrderId(e.target.value)}>{selected.workOrders.map((x) => <option value={x.id} key={x.id}>{x.title}</option>)}</select></label><label className="wide">Замечание<textarea aria-label="Замечание QC" value={comment} onChange={(e) => setComment(e.target.value)} /></label></div><div className="work-actions"><button className="secondary" disabled={!comment || command.isPending} onClick={addRework}>Добавить замечание</button><button className="secondary" disabled={command.isPending || !quality.observations.some((x) => x.requiresRework)} onClick={() => command.mutate({ path: `/api/quality-checks/${quality.id}/rework`, body: { comment: 'Вернуть выбранные работы', expectedVersion: quality.version } })}>Вернуть на исправление</button><button className="primary" disabled={command.isPending || quality.observations.some((x) => x.requiresRework)} onClick={() => command.mutate({ path: `/api/quality-checks/${quality.id}/pass`, body: { comment: 'Контроль качества пройден', expectedVersion: quality.version } })}>Подтвердить ReadyForSale</button></div></>}
    </div>}
  </section>
}
