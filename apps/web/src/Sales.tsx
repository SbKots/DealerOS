...
const newOfferLine = (): OfferLine => ({ id: crypto.randomUUID(), category: 'Оборудование', name: '', amount: 0, currency: 'RUB' })
...
export function SalesWorkspace() {
  ...
  return <>
    <div className="page-heading"><div><p className="eyebrow">Продажи</p><h1>Визиты и предложения</h1><p className="muted">Планируйте встречи и тест-драйвы, рассчитывайте предложение на сервере и сохраняйте утверждённую версию неизменной.</p></div></div>
...
function OffersWorkspace() {
  ...
  return <section className="panel sales-panel"><div className="panel-title"><span className="step">10</span><div><h2>Коммерческие предложения</h2><p>Итог и маржа рассчитываются сервером; скидка хранится отдельно от состава предложения.</p></div></div>
    {error && <div className="error-banner" role="alert">{errorText(error)}</div>}
    {!selected ? <><div className="work-form sales-form"><label className="wide">Квалифицированное обращение<select aria-label="Lead для предложения" value={leadId} onChange={(e) => { setLeadId(e.target.value); setPreview(null) }}><option value="">Выберите</option>{qualified.map((x) => <option key={x.id} value={x.id}>{x.customerName}</option>)}</select></label><OfferLinesEditor lines={lineItems} onChange={changeLine} onAdd={() => setLineItems((current) => [...current, newOfferLine()])} onRemove={(id) => setLineItems((current) => current.filter((line) => line.id !== id))} /><label>Скидка<input aria-label="Скидка" type="number" min="0" value={discount} onChange={(e) => { setDiscount(Number(e.target.value)); setPreview(null) }} /></label><div className="work-actions wide"><button className="secondary" disabled={!leadId || calculate.isPending} onClick={() => calculate.mutate()}>Рассчитать на сервере</button><button className="primary" disabled={!preview || create.isPending} onClick={() => create.mutate()}>Создать предложение</button></div></div>
      {preview && <OfferEconomics preview={preview} />}
      <div className="offer-list">{offers.data?.map((offer) => <button key={offer.id} onClick={() => selectOffer(offer)}><span><strong>{offer.customerName}</strong><small>{offer.vehicleName} · рев. {offer.revision}</small></span><strong>{money(offer.finalPriceAmount, offer.currency)}</strong><span className={`status ${offer.status === 'Approved' ? 'success' : 'draft'}`}><i />{offerStatusLabel(offer.status)}</span></button>)}</div></> : <div className="offer-card"><button className="link-button" onClick={() => setSelected(null)}>← К предложениям</button><div className="work-card-head"><div><span className={`status ${selected.status === 'Approved' ? 'success' : 'draft'}`}><i />{offerStatusLabel(selected.status)} · рев. {selected.revision}</span><h3>{selected.customerName}</h3><p>{selected.vehicleName}</p></div><strong>{money(selected.finalPriceAmount, selected.currency)}</strong></div><OfferEconomics preview={selected} />
      {['Draft', 'ChangesRequested'].includes(selected.status) && <div className="work-form sales-form"><OfferLinesEditor lines={lineItems} onChange={changeLine} onAdd={() => setLineItems((current) => [...current, newOfferLine()])} onRemove={(id) => setLineItems((current) => current.filter((line) => line.id !== id))} /><label>Скидка<input aria-label="Скидка Draft Offer" type="number" min="0" value={discount} onChange={(e) => setDiscount(Number(e.target.value))} /></label><div className="work-actions wide"><button className="secondary" disabled={update.isPending} onClick={() => update.mutate()}>Сохранить черновик</button><button className="primary" onClick={() => run('submit')}>Отправить на согласование</button></div></div>}
      {selected.status === 'Submitted' && permissions.includes('sales.offers.approve') && <div className="decision-form"><label>Обоснование<textarea aria-label="Причина решения по Offer" value={decisionReason} onChange={(e) => setDecisionReason(e.target.value)} /></label><div className="work-actions"><button className="primary" onClick={() => decide('Approved')}>Утвердить предложение</button><button className="secondary" onClick={() => decide('ChangesRequested')}>Вернуть на доработку</button><button className="danger-button" onClick={() => decide('Rejected')}>Отклонить</button></div></div>}
      {selected.approvedSnapshot && <div className="snapshot-card"><span>Утверждённая неизменяемая версия</span><strong>{money(selected.approvedSnapshot.finalPriceAmount, selected.currency)}</strong><small>Себестоимость {money(selected.approvedSnapshot.costSnapshotAmount, selected.currency)} · маржа {money(selected.approvedSnapshot.expectedMarginAmount, selected.currency)}</small><details><summary>Техническая информация</summary><pre>{selected.approvedSnapshot.lineItemsJson}</pre></details>{permissions.includes('sales.offers.edit') && <button className="secondary" onClick={() => run('revisions', { revisionId: crypto.randomUUID(), validUntil: new Date(Date.now() + 14 * 86_400_000).toISOString() })}>Создать новую ревизию</button>}</div>}
      <div className="timeline"><h3>История предложения</h3>{selected.history.map((x) => <article key={x.commandId}><span>{dateTime(x.occurredAt)}</span><strong>{x.operation}</strong></article>)}</div>
    </div>}
  </section>
}

function OfferEconomics({ preview }: { preview: OfferPreview }) {
  return <div className={`economics ${preview.belowMinimumMargin ? 'danger' : ''}`}><div><span>Публичная цена</span><strong>{money(preview.basePriceAmount, preview.currency)}</strong></div><div><span>Дополнительные позиции</span><strong>{money(preview.lineItemsAmount, preview.currency)}</strong></div><div><span>Скидка</span><strong>− {money(preview.discountAmount, preview.currency)}</strong></div><div><span>Итог</span><strong>{money(preview.finalPriceAmount, preview.currency)}</strong></div><div><span>Зафиксированная себестоимость</span><strong>{money(preview.costSnapshotAmount, preview.currency)}</strong></div><div><span>Ожидаемая маржа</span><strong>{money(preview.expectedMarginAmount, preview.currency)}</strong></div>{preview.requiresManagerApproval && <p className="wide">Требуется согласование руководителя: скидка выше {money(preview.autoApprovalDiscountLimit, preview.currency)} или маржа ниже установленной границы.</p>}</div>
}
...
function visitStatusLabel(status: string) { return ({ Scheduled: 'Запланирован', Arrived: 'Клиент прибыл', Completed: 'Завершён', NoShow: 'Не состоялся', Cancelled: 'Отменён' } as Record<string, string>)[status] ?? status }
function offerStatusLabel(status: string) { return ({ Draft: 'Черновик', Submitted: 'На согласовании', ChangesRequested: 'На доработке', Approved: 'Утверждено', Rejected: 'Отклонено', Cancelled: 'Отменено' } as Record<string, string>)[status] ?? status }
