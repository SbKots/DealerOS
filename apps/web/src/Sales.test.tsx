...
describe('SalesWorkspace', () => {
  ...
  it('runs the explicit arrival and safe test-drive checkout commands', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: 'Выдать на test drive' }))
  ...
  it('renders totals only from the server preview and explains manager approval', async () => {
    ...
    expect(screen.getByText(/Требуется manager approval/)).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Создать Offer' })).toBeEnabled()
  })
  ...
  it('requires an explicit reason and shows the immutable approved snapshot', async () => {
    ...
    await userEvent.click((await screen.findByText('Lada XRAY · rev. 1')).closest('button')!)
    expect(screen.getByLabelText('Причина решения по Offer')).toHaveValue('Экономика и полномочия проверены')
    await userEvent.click(screen.getByRole('button', { name: 'Утвердить Offer' }))
    expect(await screen.findByText('Immutable Approved Offer')).toBeInTheDocument()
  })
