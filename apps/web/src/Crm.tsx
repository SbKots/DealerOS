...
export function CrmWorkspace() {
  ...
  return <>
    <div className="page-heading"><div><p className="eyebrow">Итерация 0.6</p><h1>Клиенты и первый ответ</h1><p className="muted">Единый inbox, безопасное объединение дублей и измеримый SLA до первого осмысленного контакта.</p></div><div className="metric"><span>SLA просрочен</span><strong>{(leads.data ?? []).filter((x) => x.slaBreached).length}</strong><small>лидов требуют внимания</small></div></div>
    ...
    <div className="crm-grid">
      <section className="panel"><div className="panel-title"><span className="step">07</span><div><h2>Реестр клиентов</h2><p>Контакты нормализуются сервером; совпадения не объединяются автоматически.</p></div></div>
        ...
        {mergePreview && <div className="warning-banner"><strong>Будет перемещено лидов: {mergePreview.leadsToMove}</strong><span>{mergePreview.warning}</span><label>Причина объединения<input aria-label="Причина объединения" value={mergeReason} onChange={(e) => setMergeReason(e.target.value)} /></label><button className="danger-button" disabled={!mergeReason || merge.isPending} onClick={() => merge.mutate()}>Подтвердить объединение с {mergePreview.target.name}</button></div>}
      </section>

      <section className="panel"><div className="panel-title"><span className="step">08</span><div><h2>Inbox лидов</h2><p>Назначение и каждое изменение статуса выполняются отдельной командой.</p></div></div>
        <div className="filter-row"><label>Филиал<select aria-label="Фильтр филиала" value={branchFilter} onChange={(e) => setBranchFilter(e.target.value)}><option value="All">Все</option>{branches.data?.map((x) => <option value={x.id} key={x.id}>{x.name}</option>)}</select></label><label>Менеджер<select aria-label="Фильтр менеджера" value={managerFilter} onChange={(e) => setManagerFilter(e.target.value)}><option value="All">Все</option>{leadManagers.map((x) => <option value={x.id} key={x.id}>{x.name}</option>)}</select></label><label>Статус<select aria-label="Фильтр лидов" value={status} onChange={(e) => setStatus(e.target.value)}><option>Active</option><option>Overdue</option><option>New</option><option>Assigned</option><option>FirstContact</option><option>Qualified</option><option>Lost</option><option>All</option></select></label></div>
        {selectedCustomer && permissions.includes('crm.leads.create') && <div className="work-form crm-form"><p className="wide"><strong>Новый лид: {selectedCustomer.name}</strong></p><label>Филиал<select aria-label="Филиал лида" value={leadDraft.branchId || selectedCustomer.createdInBranchId || firstBranch} onChange={(e) => setLeadDraft({ ...leadDraft, branchId: e.target.value })}>{branches.data?.map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select></label><label>Автомобиль<select aria-label="Автомобиль лида" value={leadDraft.vehicleId} onChange={(e) => setLeadDraft({ ...leadDraft, vehicleId: e.target.value })}><option value="">По параметрам</option>{vehicles.data?.map((x) => <option key={x.id} value={x.id}>{x.make} {x.model}</option>)}</select></label><label>Источник<input aria-label="Источник лида" value={leadDraft.source} onChange={(e) => setLeadDraft({ ...leadDraft, source: e.target.value })} /></label><label>Параметры поиска<input aria-label="Параметры поиска" disabled={Boolean(leadDraft.vehicleId)} value={leadDraft.searchCriteria} onChange={(e) => setLeadDraft({ ...leadDraft, searchCriteria: e.target.value })} /></label><button className="secondary" disabled={!leadDraft.source || (!leadDraft.vehicleId && !leadDraft.searchCriteria) || createLead.isPending} onClick={() => createLead.mutate()}>Создать лид</button></div>}
        {!selectedLead ? <div className="lead-list">{leads.isLoading ? <p>Загрузка…</p> : visibleLeads.length ? visibleLeads.map((lead) => <button className={`lead-row ${lead.slaBreached ? 'overdue' : ''}`} key={lead.id} onClick={() => setSelectedLead(lead)}><span><strong>{lead.customerName}</strong><small>{lead.vehicleId ? 'Выбран автомобиль' : lead.searchCriteria} · {lead.source}</small></span><span><b>{lead.status}</b><small>{lead.slaBreached ? 'SLA просрочен' : `до ${dateTime(lead.firstResponseDueAt)}`}</small></span></button>) : <div className="empty-state compact"><strong>В выбранном фильтре лидов нет</strong></div>}</div> : <LeadCard lead={selectedLead} managers={managers.data ?? []} managerId={managerId} setManagerId={setManagerId} summary={summary} setSummary={setSummary} nextAction={nextAction} setNextAction={setNextAction} closeReason={closeReason} setCloseReason={setCloseReason} pending={command.isPending} onBack={() => setSelectedLead(null)} onRoundRobin={() => run('assign-round-robin', { commandId: crypto.randomUUID(), expectedVersion: selectedLead.version })} onAssign={() => run('assign', { commandId: crypto.randomUUID(), managerUserId: managerId, expectedVersion: selectedLead.version })} onContact={() => run('activities', { commandId: crypto.randomUUID(), type: 'Call', direction: 'Outbound', result: 'Answered', summary, meaningfulContact: true, nextAction, nextActionDueAt: dueTomorrow(), expectedVersion: selectedLead.version })} onQualify={() => run('qualify', { commandId: crypto.randomUUID(), nextAction, nextActionDueAt: dueTomorrow(), expectedVersion: selectedLead.version })} onClose={() => run('close', { commandId: crypto.randomUUID(), status: 'Lost', reason: closeReason, expectedVersion: selectedLead.version })} />}
      </section>
    </div>
  </>
}
...
function LeadCard(props: { lead: Lead; managers: Manager[]; managerId: string; setManagerId: (x: string) => void; summary: string; setSummary: (x: string) => void; nextAction: string; setNextAction: (x: string) => void; closeReason: string; setCloseReason: (x: string) => void; pending: boolean; onBack: () => void; onRoundRobin: () => void; onAssign: () => void; onContact: () => void; onQualify: () => void; onClose: () => void }) {
  ...
  return <div className="lead-card"><button className="link-button" onClick={props.onBack}>← К inbox</button><div className="work-card-head"><div><span className={`status ${lead.slaBreached ? 'danger' : 'draft'}`}><i />{lead.status}{lead.slaBreached ? ' · SLA просрочен' : ''}</span><h3>{lead.customerName}</h3><p>{lead.searchCriteria}</p></div><div><small>Менеджер</small><strong>{lead.assignedManagerName ?? 'Не назначен'}</strong></div></div>
    ...
    {!['Lost', 'Spam', 'Duplicate'].includes(lead.status) && <div className="work-actions"><label>Причина потери<input aria-label="Причина потери" value={props.closeReason} onChange={(e) => props.setCloseReason(e.target.value)} /></label><button className="secondary" disabled={!props.closeReason || props.pending} onClick={props.onClose}>Закрыть как Lost</button></div>}
    <div className="timeline"><h3>Таймлайн</h3>{lead.activities.length ? lead.activities.map((x) => <article key={x.id}><span>{dateTime(x.createdAt)}</span><strong>{x.type} · {x.direction}</strong><p>{x.summary}</p></article>) : <p className="muted">Контактов пока нет.</p>}</div>
  </div>
}
