...
export function DealsWorkspace() {
  ...
  return <>
    <div className="page-heading"><div><p className="eyebrow">Продажи</p><h1>Сделка, оплата и выдача</h1><p className="muted">Сделка фиксирует утверждённое предложение. Платежи и возвраты сохраняются в истории, а документы доступны только сотрудникам с правами.</p></div><div className="metric"><span>Сделки</span><strong>{deals.data?.length ?? 0}</strong><small>в доступных филиалах</small></div></div>
    {error && <div className="error-banner workspace-error" role="alert">{errorText(error)}</div>}
    <div className="deal-layout">
      <section className="panel"><div className="panel-title"><span className="step">13</span><div><h2>Бронирование → сделка</h2><p>Преобразование бронирования и блокировка автомобиля выполняются одной серверной командой.</p></div></div>
        {permissions.includes('deals.create') && <div className="work-form deal-create"><label className="wide">Активное бронирование<select aria-label="Активное бронирование для сделки" value={reservationId} onChange={(event) => setReservationId(event.target.value)}><option value="">Выберите бронь</option>{activeReservations.map((item) => <option key={item.id} value={item.id}>{item.customerName} · {item.vehicleName}</option>)}</select></label><button className="primary" disabled={!reservationId || create.isPending} onClick={() => create.mutate()}>Создать сделку</button></div>}
        {!selected ? <div className="deal-list">{deals.data?.map((deal) => <button key={deal.id} onClick={() => refresh(deal)}><span><strong>{deal.customerName}</strong><small>{vehicleName(deal.vehicleSnapshotJson)}</small></span><strong>{money(deal.finalTotalAmount, deal.currency)}</strong><span className={`status ${deal.status === 'Completed' ? 'success' : deal.status.includes('Refund') || deal.status === 'Cancelled' ? 'danger' : 'draft'}`}><i />{dealStatusLabel(deal.status)}</span></button>)}</div> : <button className="link-button" onClick={() => setSelected(null)}>← К списку сделок</button>}
      </section>

      {selected && <section className="panel deal-workspace"><div className="work-card-head"><div><span className={`status ${selected.status === 'Completed' ? 'success' : 'draft'}`}><i />{dealStatusLabel(selected.status)}</span><h2>{selected.customerName}</h2><p>{vehicleName(selected.vehicleSnapshotJson)}</p></div><strong>{money(selected.finalTotalAmount, selected.currency)}</strong></div>
        ...
        {selected.status === 'AwaitingPayment' && <div className="deal-sections"><div className="decision-form"><h3>Реестр платежей</h3><label>Сумма<input aria-label="Сумма платежа" type="number" min="0.01" value={paymentAmount} onChange={(event) => setPaymentAmount(Number(event.target.value))} /></label><label>Причина<input aria-label="Причина платежа" value={reason} onChange={(event) => setReason(event.target.value)} /></label><button className="primary" disabled={paymentAmount <= 0} onClick={() => registerPayment('Payment', paymentAmount)}>Зарегистрировать оплату</button></div><DocumentActions deal={selected} generate={generate} download={download} /><button className="primary" onClick={() => run('ready-for-handover')}>Проверить готовность к выдаче</button></div>}
        {selected.status === 'ReadyForHandover' && <div className="decision-form"><h3>Контрольный лист выдачи</h3><label>Фактический пробег<input aria-label="Фактический пробег при выдаче" type="number" value={mileage} onChange={(event) => setMileage(Number(event.target.value))} /></label><p className="info-banner">Ключи, документы, оборудование, состояние и ответственные подтверждаются в неизменяемой записи.</p>{!selected.handover ? <button className="primary" onClick={() => run('handover', { actualMileageKm: mileage, keysTransferred: true, documentsTransferred: true, equipmentTransferred: true, conditionConfirmed: true, issuerConfirmed: true, responsibleConfirmed: true, conditionNotes: 'Состояние соответствует Deal snapshot', comments: 'Demo handover' })}>Завершить контрольный лист выдачи</button> : <button className="primary" onClick={() => run('complete')}>Выдать автомобиль и завершить сделку</button>}</div>}
        {selected.status === 'RefundPending' && <div className="decision-form"><h3>Требуется возврат</h3><p>Автомобиль останется в процессе продажи до полного возврата {money(selected.netPaid, selected.currency)}.</p><button className="danger-button" onClick={() => registerPayment('Refund', selected.netPaid)}>Зафиксировать полный возврат</button></div>}
        ...
        {selected.status === 'Completed' && <div className="success-banner"><span>✓</span><div><strong>Автомобиль продан</strong><small>Сделка и документы доступны только для чтения; активная публикация снята.</small></div></div>}
        {selected.status !== 'AwaitingPayment' && <DocumentActions deal={selected} generate={generate} download={download} readOnly />}
        <div className="payment-timeline"><h3>Платежи и возвраты</h3>{selected.payments.map((item) => <article key={item.id}><span>{dateTime(item.occurredAt)}</span><strong>{paymentKindLabel(item.kind)} · {paymentStatusLabel(item.status)}</strong><b>{money(item.amount, item.currency)}</b><small>{item.manualReference}</small></article>)}</div>
      </section>}
    </div>
  </>
}
...
function vehicleName(json: string) { try { const value = JSON.parse(json) as { make?: string; model?: string; vin?: string }; return `${value.make ?? ''} ${value.model ?? ''} · ${value.vin ?? ''}`.trim() } catch { return 'Зафиксированные данные автомобиля' } }
function dateTime(value: string) { return new Date(value).toLocaleString('ru-RU', { dateStyle: 'short', timeStyle: 'short' }) }
function dealStatusLabel(status: string) { return ({ Draft: 'Черновик', AwaitingPayment: 'Ожидает оплаты', ReadyForHandover: 'Готова к выдаче', Completed: 'Завершена', Cancelled: 'Отменена', RefundPending: 'Ожидает возврата', Refunded: 'Возвращена' } as Record<string, string>)[status] ?? status }
function paymentKindLabel(kind: string) { return ({ Payment: 'Оплата', Refund: 'Возврат' } as Record<string, string>)[kind] ?? kind }
function paymentStatusLabel(status: string) { return ({ Received: 'Получена', Refunded: 'Возвращена', Failed: 'Не проведена' } as Record<string, string>)[status] ?? status }
