...
export function ReconditioningWorkspace() {
  ...
  if (activeId) {
    ...
    return <div className="reconditioning-workspace">
      ...
      <div className="page-heading plan-heading"><div><p className="eyebrow">План предпродажной подготовки</p><h1>{plan.make} {plan.model}</h1><p className="muted">{plan.branchName} · Автор: {plan.createdByName} · Осмотр {plan.sourceInspectionId.slice(0, 8)}</p></div><BudgetCards budgets={plan.budgets} approved={snapshot?.approvedLimitAmount} /></div>
...
function WorkEditor({ work, editable, busy, onSave, onRemove }: { work: ReconditioningWork; editable: boolean; busy: boolean; onSave: (body: object) => void; onRemove: (reason?: string) => void }) {
  ...
  return <article className={`work-card ${work.isMandatory ? 'mandatory' : ''}`}>
    ...
    <div className="work-fields">
      ...
      <label>Категория<select aria-label={`Категория работы: ${work.sourceDefectTitle}`} disabled={!editable} value={draft.category} onChange={(e) => set('category', e.target.value)}>{categories.map((x) => <option key={x}>{x}</option>)}</select></label>
      <label>Приоритет<select aria-label={`Приоритет работы: ${work.sourceDefectTitle}`} disabled={!editable} value={draft.priority} onChange={(e) => set('priority', e.target.value)}>{priorities.map((x) => <option key={x}>{x}</option>)}</select></label>
      <label>Исполнитель<select aria-label={`Тип исполнителя: ${work.sourceDefectTitle}`} disabled={!editable} value={draft.executorType} onChange={(e) => set('executorType', e.target.value)}><option>Internal</option><option>External</option></select></label>
