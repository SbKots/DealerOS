...
export function OperationsWorkspace() {
  ...
  return <>
    <div className="page-heading"><div><p className="eyebrow">Автомобили</p><h1>Выполнение подготовки</h1><p className="muted">Работы, фактические расходы и отклонение от утверждённого лимита.</p></div><div className="metric"><span>Готово к запуску</span><strong>{approved.length}</strong><small>утверждённых планов</small></div></div>
    {create.error && <div className="error-banner" role="alert">{errorText(create.error)}</div>}
    <section className="panel"><div className="panel-title"><span className="step">04</span><div><h2>Очередь исполнения</h2><p>Выполнение создаётся только из неизменяемого снимка утверждённого бюджета.</p></div></div>
...
function WorkOrderCard({ execution, work, busy, onCommand }: { execution: ReconditioningExecution; work: ExecutionWorkOrder; busy: boolean; onCommand: (path: string, body: object) => void }) {
  ...
  return <article className="work-card"><div className="work-card-head"><div><span className="status draft"><i />{workStatusLabel(work.status)}</span><h3>{work.title}</h3><p>{executorTypeLabel(work.executorType)} · срок {new Date(work.dueAt).toLocaleDateString('ru-RU')}</p></div><strong>{money(work.actualLaborAmount + work.actualMaterialAmount + work.actualExternalAmount, work.currency)}</strong></div>
...
function executionStatusLabel(status: string) { return ({ Draft: 'Черновик', InProgress: 'В работе', Completed: 'Завершено', Cancelled: 'Отменено' } as Record<string, string>)[status] ?? status }
function workStatusLabel(status: string) { return ({ Scheduled: 'Запланировано', InProgress: 'В работе', Completed: 'Завершено', Cancelled: 'Отменено' } as Record<string, string>)[status] ?? status }
function executorTypeLabel(type: string) { return ({ Internal: 'Внутренний исполнитель', External: 'Подрядчик' } as Record<string, string>)[type] ?? type }
