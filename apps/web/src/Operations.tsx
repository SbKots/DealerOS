...
function WorkOrderCard({ execution, work, busy, onCommand }: { execution: ReconditioningExecution; work: ExecutionWorkOrder; busy: boolean; onCommand: (path: string, body: object) => void }) {
  ...
  return <article className="work-card"><div className="work-card-head"><div><span className="status draft"><i />{work.status}</span><h3>{work.title}</h3><p>{work.executorType} · срок {new Date(work.dueAt).toLocaleDateString('ru-RU')}</p></div><strong>{money(work.actualLaborAmount + work.actualMaterialAmount + work.actualExternalAmount, work.currency)}</strong></div>
    <div className="work-form"><label>Часы<input aria-label={`Часы: ${work.title}`} type="number" min="0" step="0.25" disabled={!mutable} value={laborHours} onChange={(e) => setLaborHours(Number(e.target.value))} /></label><label>Работа<input aria-label={`Факт работы: ${work.title}`} type="number" min="0" disabled={!mutable} value={labor} onChange={(e) => setLabor(Number(e.target.value))} /></label><label>Подрядчик<input aria-label={`Факт подрядчика: ${work.title}`} type="number" min="0" disabled={!mutable} value={external} onChange={(e) => setExternal(Number(e.target.value))} /></label><label>Материал<input aria-label={`Материал: ${work.title}`} type="number" min="0" disabled={!mutable} value={material} onChange={(e) => setMaterial(Number(e.target.value))} /></label></div>
