...
describe('DealsWorkspace', () => {
  ...
  it('creates a deal only from an active reservation', async () => {
    ...
    await userEvent.selectOptions(screen.getByLabelText('Active Reservation для сделки'), reservation.id)
    await userEvent.click(screen.getByRole('button', { name: 'Создать Deal snapshot' }))
