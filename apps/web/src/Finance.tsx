...
export function FinanceWorkspace() {
  ...
  return <>
    <div className="page-heading"><div><p className="eyebrow">DealerOS 1.0</p><h1>Экономика проданных автомобилей</h1><p className="muted">Immutable ProfitSnapshot связывает план и факт с исходными Deal, Operations, refund и cost records.</p></div><div className="metric"><span>Продано</span><strong>{groups.reduce((sum, x) => sum + x.soldVehicles, 0)}</strong><small>в выбранном периоде</small></div></div>
    <section className="panel finance-filters"><label>С даты<input aria-label="Финансы с даты" type="date" value={from} onChange={(event) => setFrom(event.target.value)} /></label><label>По дату<input aria-label="Финансы по дату" type="date" value={to} onChange={(event) => setTo(event.target.value)} /></label><label>Валюта<select aria-label="Валюта dashboard" value={currency} onChange={(event) => setCurrency(event.target.value)}><option value="">Все, раздельно</option><option value="RUB">RUB</option><option value="USD">USD</option><option value="EUR">EUR</option></select></label>{permissions.includes('finance.export') && <button className="secondary" onClick={exportCsv}>Экспорт CSV</button>}</section>
    {(query.error || detail.error) && <div className="error-banner workspace-error" role="alert">{errorText(query.error ?? detail.error)}</div>}
    {query.isLoading ? <section className="panel"><p>Загрузка экономики…</p></section> : groups.length === 0 ? <section className="panel empty-state"><strong>Нет завершённых сделок</strong><p>После выдачи автомобиля здесь появится ProfitSnapshot.</p></section> : groups.map((group) => <section className="panel finance-group" key={group.currency}>
      ...
      <div className="economics finance-metrics"><Metric label="Gross revenue" value={group.grossRevenue} currency={group.currency} /><Metric label="Net revenue" value={group.netRevenue} currency={group.currency} /><Metric label="Total cost" value={group.totalCost} currency={group.currency} /><Metric label="Actual profit" value={group.actualProfit} currency={group.currency} danger={group.actualProfit < 0} /><div><span>Средняя маржа</span><strong>{group.averageMarginPercent == null ? '—' : `${group.averageMarginPercent}%`}</strong></div><Metric label="Plan/fact" value={group.planFactProfitVariance} currency={group.currency} danger={group.planFactProfitVariance < 0} /></div>
...
function Source({ label, value, snapshot }: { label: string; value: number; snapshot: Snapshot }) { return <article><span>{label}</span><strong>{money(value, snapshot.currency)}</strong><small>Источник зафиксирован в snapshot rev. {snapshot.revision}</small></article> }
function money(value: number, currency: string) { return new Intl.NumberFormat('ru-RU', { style: 'currency', currency }).format(value) }
function errorText(error: unknown) { return error instanceof ApiError ? error.message : 'Не удалось загрузить финансовый dashboard.' }
