...
export function ReservationsWorkspace() {
  ...
  return <>
    <div className="page-heading"><div><p className="eyebrow">Итерация 0.8</p><h1>Брони автомобилей</h1><p className="muted">Бронь создаётся только из действующего Approved Offer. PostgreSQL атомарно защищает автомобиль от второго клиента.</p></div><div className="metric"><span>Активные</span><strong>{reservations.data?.filter((x) => ['PendingDeposit', 'Active'].includes(x.status)).length ?? 0}</strong><small>временных блокировок автомобиля</small></div></div>
    ...
    <div className="reservation-layout">
      <section className="panel"><div className="panel-title"><span className="step">11</span><div><h2>Новая бронь</h2><p>Клиент и автомобиль берутся из неизменяемого предложения.</p></div></div>
        {permissions.includes('reservations.create') ? <div className="work-form reservation-form">
          <label className="wide">Approved Offer<select aria-label="Approved Offer для брони" value={snapshotId} onChange={(event) => setSnapshotId(event.target.value)}><option value="">Выберите предложение</option>{approvedOffers.map((offer) => <option key={offer.approvedSnapshot!.id} value={offer.approvedSnapshot!.id}>{offer.customerName} · {offer.vehicleName} · {money(offer.finalPriceAmount, offer.currency)}</option>)}</select></label>
        ...
        {!approvedOffers.length && !offers.isLoading && permissions.includes('reservations.create') && <div className="empty-state compact"><strong>Нет доступных Approved Offer</strong><p>Сначала завершите визит и утвердите предложение.</p></div>}
      </section>
